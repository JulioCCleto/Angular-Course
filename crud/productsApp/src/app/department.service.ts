...
export class DepartmentService {
  ...
  private departmentsSubjec$: BehaviorSubject<Department[]> = new BehaviorSubject<Department[]>(null);
  ...
  get(): Observable<Department[]> {
    if (!this.loaded) {
      this.http.get<Department[]>(this.url)
        ...
        .subscribe(this.departmentsSubjec$);
      this.loaded = true;
    }
    return this.departmentsSubjec$.asObservable();
  }

  add(d: Department): Observable<Department> {
    return this.http.post<Department>(this.url, d)
      .pipe(
        tap((dep: Department) => this.departmentsSubjec$.getValue().push(dep))
      );
  }
  ...
  del(dep: Department): Observable<any> {
    return this.http.delete(`${this.url}/${dep._id}`)
      .pipe(tap(() => {
        let departments = this.departmentsSubjec$.getValue();
